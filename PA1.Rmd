...
median_stepsperday
```
Average steps taken per day are `r mean_stepsperday` and median steps per day are 'r median_stepsperday` .
...
```
The interval `r maxnumsteps[1]` has, on average, the highest count of steps, with `r maxnumsteps[2]` steps.
...
When imputing missing data with the average number of steps in the same 5-min interval, both the mean and the median have the same value,`r mean_stepsperday_newset`.
